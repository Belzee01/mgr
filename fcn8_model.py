...
class FCN_8:
    ...
    def create(input_shape: Tuple[int, int, int], n_classes=1, base=4, predefined=False):
        ...
        s = tf.keras.layers.Lambda(lambda x: x / 255)(i)

        ## Block 1
        x = Conv2D(2 ** base, (3, 3), activation='elu', padding='same', name='block1_conv1')(s)
        x = Conv2D(2 ** base, (3, 3), activation='elu', padding='same', name='block1_conv2')(x)
        x = MaxPooling2D((2, 2), strides=(2, 2), name='block1_pool')(x)
        f1 = x

        # Block 2
        x = Conv2D(2 ** (base + 1), (3, 3), activation='elu', padding='same', name='block2_conv1')(x)
        x = Conv2D(2 ** (base + 1), (3, 3), activation='elu', padding='same', name='block2_conv2')(x)
        x = MaxPooling2D((2, 2), strides=(2, 2), name='block2_pool')(x)
        f2 = x

        # Block 3
        x = Conv2D(2 ** (base + 2), (3, 3), activation='elu', padding='same', name='block3_conv1')(x)
        x = Conv2D(2 ** (base + 2), (3, 3), activation='elu', padding='same', name='block3_conv2')(x)
        x = Conv2D(2 ** (base + 2), (3, 3), activation='elu', padding='same', name='block3_conv3')(x)
        x = MaxPooling2D((2, 2), strides=(2, 2), name='block3_pool')(x)
        pool3 = x

        # Block 4
        x = Conv2D(2 ** (base + 3), (3, 3), activation='elu', padding='same', name='block4_conv1')(x)
        x = Conv2D(2 ** (base + 3), (3, 3), activation='elu', padding='same', name='block4_conv2')(x)
        x = Conv2D(2 ** (base + 3), (3, 3), activation='elu', padding='same', name='block4_conv3')(x)
        pool4 = MaxPooling2D((2, 2), strides=(2, 2), name='block4_pool')(x)

        # Block 5
        x = Conv2D(2 ** (base + 3), (3, 3), activation='elu', padding='same', name='block5_conv1')(pool4)
        x = Conv2D(2 ** (base + 3), (3, 3), activation='elu', padding='same', name='block5_conv2')(x)
        x = Conv2D(2 ** (base + 3), (3, 3), activation='elu', padding='same', name='block5_conv3')(x)
...
# Model
model = FCN_8.create(input_shape=INPUT_SHAPE)
